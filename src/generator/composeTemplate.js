...
export const DIFFICULTY_LABELS = { easy: '쉬움', normal: '보통' };
...
const SNAKE_LENGTH_RANGES = { easy: [5, 7], normal: [7, 10] };
const TURNTABLE_SIZE_RANGES = { easy: [3, 3], normal: [3, 4] };

/**
 * 보드 개수와 난이도에 비례해 스네이크를 몇 개 배치할지 정한다 — 쉬움은 최소(1개),
 * 보통은 보드 절반쯤. 보드당 최대 1개까지만 배치해서, 같은 종류의 규칙끼리는 애초에
 * 같은 보드를 두고 다툴 일이 없게 한다(다른 보드끼리 3x3 모서리를 공유하는 경우의 충돌은
 * prefillSnakeWalks가 이미 배치한 자리를 reservedKeys로 피해가며 처리한다).
 */
function scaledSnakeCount(boardCount, difficulty) {
  if (difficulty === 'easy') return 1;
  return Math.max(1, Math.ceil(boardCount / 2)); // normal
}
...
export function buildTemplateFromSelection(resolved) {
  ...
  if (resolved.elements.turntable) {
    const count = 1; // 보드마다 다 돌리게 하면 피로감이 커서(회전 조작이 손이 많이 감) 하나로 묶어둔다
